...
class PageSetupController extends GetxController {
  ...
  Future<void> setPresence({
    ...
    bool inArea = false;
    dynamic currentDevice = await getUserLastestDevice();
    ...
    if (getYesterdayData.exists &&
        ...
        !getTodayData.exists) {
      print(
          "/-->Absen Pulang Sip3 saat kemarin lembur sip3 hari ini blm absen.");
      ...
      if (!getYesterdayData.exists) {
        ...
        if (lastDayData?["pulang"] == null) {
          ...
          if (!getDupeYesterdayData.exists) {
            ...
            if (!getTodayData.exists && yesterdayDupeData?['pulang'] != null) {
              //Absen Masuk Normal
              print("//---------------->Absen Masuk Normal.");
  ...
  Future<String> getUserLastestDevice() async {
    ...
    String? userDevice = await firestore
        .collection('users')
        .doc(uid)
        .get()
        .then((value) => value.data()?['device']);
  ...
  getDeviceInfo() async {
    ...
    if (deviceId.isEmpty) {
      return Get.defaultDialog(
          middleText:
              "Your new Device ID is : $currentDeviceId \n Your new Device Name is : $deviceModel at ${now.toIso8601String()}",
    ...
    if (deviceId != currentDeviceId) {
      return Get.defaultDialog(
          middleText:
              "Your Device ID is : $deviceId \n Your Device Name is : $deviceModel at ${now.toIso8601String()}",
          confirm: ElevatedButton(
              onPressed: () {
                Get.back();
                Get.back();
